...
{
    public sealed class ObjectInvisibleLavaPlane : SpecialObjectCreator
    {
        public ObjectInvisibleLavaPlane(NetworkObject reference) : base(reference)
        {
        }
        ...
        {
            if (!prefabId.Equals(AssetPaths.INVISIBLE_LAVA_PLANE_ID, StringComparison.Ordinal))
            {
                Debug.LogError(
                    $"ObjectInvisibleLavaPlane error, prefabId: {prefabId} : created object is {AssetPaths.INVISIBLE_LAVA_PLANE_ID}");
                return;
            }
            
            await CreateSpecialObject<InvisibleLavaPlane>(Reference, p, r, s, parent);
        }
    }
}