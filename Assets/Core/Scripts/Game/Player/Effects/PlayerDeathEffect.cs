...
namespace Core.Scripts.Game.Player.Effects
{
    public sealed class PlayerDeathEffect : MonoBehaviour
    {
        ...
        [SerializeField] private ParticleSystem _lineVfx;
        ...
        private void CreateVisualEffect(Vector3 position, Vector3 rotation, Vector3 scale,
            bool isLine = false)
        {
            if (_lineVfx == null || _lavaVfx == null) return;

            Transform fx = isLine ? Instantiate(_lineVfx).transform : Instantiate(_lavaVfx).transform;
